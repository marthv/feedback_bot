  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        source: "slackbot",
        // Xano gates on a `secret` body field, matching the pattern used by the
        // other secret-gated endpoints in that workspace. The X-Api-Key header
        // above is sent too but is currently ignored server-side.
        secret: process.env.XANO_API_KEY || "",
      }),
