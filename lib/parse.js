...
export function parseEditCommand(raw) {
  ...
  // Admin "view as user". Takes an email or a numeric user id, plus an optional
  // reason after "--" which is stored in admin_audit rather than shown in Slack.
  // Deliberately NOT folded into the edit parser: it shares no shape with it.
  if (lower === "view-as" || lower === "viewas") {
    const remainder = rest.join(" ").trim();
    const [targetRaw, ...noteParts] = remainder.split("--");
    const target = (targetRaw || "").trim();
    if (!target) {
      return {
        action: "error",
        error: "Who am I viewing as? Try `/tulle view-as sara@example.com`.",
      };
    }
    return { action: "view-as", target, note: noteParts.join("--").trim() || undefined };
  }
