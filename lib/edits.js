...
/**
 * One HTTP helper for every Xano call in this bot.
 *
 * The secret is a PARAMETER rather than a hardcoded env read because not every
 * endpoint shares one. The staged-edit endpoints all sit behind XANO_API_KEY;
 * admin view-as deliberately has its own, because it is a much higher-privilege
 * capability than proposing a vendor edit. Copying this function to give it a
 * different secret would leave two copies to drift apart.
 */
export async function call(path, body, method = "POST", { secret = process.env.XANO_API_KEY } = {}) {
  const base = apiBase();
